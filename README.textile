...
h2. What's New for v1.1

* Completely new Plugin Configurator
* Add Command to enable user copy tags to user directory
* Support even more simplified tag usage
* Add comments to sample application application.conf file
* Support zero configuration
* Document improvement

h2. What's New for v1.0
...
# Add GreenScript module in your application.conf file
...
h3. <a>Configure GreenScript Plugin</a>
...
greenscript.url.minimized=/gs

p. Note that js and css url is used only when greenscript.miminize set to false, in which case, GreenScript will output links refer the original javascript/css files.

p. greenscript.url.minimized setting is used only when greenscript.minimize set to true, in which case, GreenScript will output links refer to the compressed(minimized) files
...
p. Javascript/css dependencies are documented in a separate configuration file named "greenscript.conf", which should be put into the conf dir (the same place with your application.conf)

p. It's fairly straght forward to document the file dependencies. Let's say your have four javascript files a.js, b.js, c.js and d.js, the dependency relationship is b.js depends on a.js, c.js depends on both b.js and d.js, then here is the content of your <code>greenscript.conf</code> file:

bc. js.b=a
js.c=b,d

p. The same way applies to css file dependencies. The only difference is css dependancy rule starts with "css." while javascript file rule starts with "js.". Below is the content of <code>greenscript.conf</code> file of the demo application:
...
** When you put "loadAll: true" in #{greenscript.js} tag, it will output all unloaded js dependency files as well as the default js file you've defined in <code>greenscript.conf</code>. I am sure jquery-1.4.2.min.js will be reached by either of the 2 lookup paths, otherwise, I assume you will not need that file. For perfectionist, here is how to load the file anyway: <code>#{greenscript.js "jquery-1.4.2.min.js", output: true/}
* How can I get "moreStyles" and "moreScripts"?
** You get them automatically when you have "output: true" for #{greenscript.css} or "loadAll: true" for #{greenscript.js}. The assumption is you have tell GreenScript that you need them in other places. I will let you know how to do that later at next section.
...
p. The differences of using GreenScript tag in other templates and the "main.html" is that ususally you don't "output" javascript or css files in your other templates, instead, you declare you need them. Here is a sample (found in ${play.path}/samples-and-tests/booking/app/views/Hotels/book.html) of how to declare javascripts and css when you don't have GreenScript:
...
p. Easy, right? You might noticed that I have put the full path for the css file in this case. This is needed because the file is not in the default stylesheet file folder (configured with <code>greenscript.dir.css</code>, which default to /public/stylesheets).

h3. <a>Configuration at runtime</a>

p. The beautiful feature enable app developer to turn on/off minimizing dynamically and could be very helpful when you need to debug your javascript/css. In order to use the feature, you will need to add an entry in your route file to map a url request to the controllers.greenscript.Configurator actions:

bc. # Enable GreenScript dynamic configuration
GET /gs module:greenscript

p. Once you have done that, you can reach the configuration page by typing "http://localhost:9000/gs" in the address bar of your favorite browser. The configuration is designed to be self guided and you won't lost yourself there.
...
* Option 2: If you are a real hack and reject any manual operations, you will probably implement your own controller extends (or @With) controllers.greenscript.Configurator, and then add security to your controller. You will need to copy the configurator templates to your views folder. Don't worry, GreenScript provides command to help you with that. I will get there soon.

h3. <a>Module command</a>

p. I just told you that you can use command to copy the greenscript Configurator.configure.html template file to your app folder. Here is how to do it. First make sure you have enabled greenscript in your application.conf file. And then go to the console, enter your app root dir, and type:
...
p. Now guess what happened? You are right, it copied the tags from module folder to your app folder: your-app-root/app/views/tag/gs. And you are free to use the simplified version: #{gs.js "js1 js2 ..." /} and #{gs.css "css1 ..." /}. What? you are still not satisfied? how come? it's already shorter than play's #{script} tag! Okay, here is my nuclear weapon: 
...
<h3>About zero configuration</h3>

<p>GreenScript plugin now support zero configuration. It means besides enabling it in your application.conf, you don't need to do any configurations, you don't even need to create "greenscript.conf" file in your conf dir. But what do you get if you don't do any configuration? Well basically you can still benefit from the brief greenscript tag syntax, you are free to use all the knowledges you've learned from <a href="#using-tags">Using Tags</a> section. You can also benefit from minimizing/compressing/cache, you can also use the dynamic configuration controller. The only thing you need to take care is your dependencies. When you are declare a javascript or css file in a template, you should also make sure all its dependencies are explicitly declared before that scripts "IN PLACE"! If you failed to do that, you might get a lot of script/css errors in your final rendered html page.

<p>As an example to demonstrate zero configuration, I put the ${play.path}/samples-and-tests/booking sample in the samples-and-tests dir of greenscript, makes the minimum changes to the templates and application.conf files.

<p>In the end, I don't think Greenscript is a heavy-weight plugin, 

p. Thanks for the great "Playframework"!